...
from PreProcessor.Service import Drawing, Validators
from Processor import MainProcessing
from collections import defaultdict
from tkinter import messagebox
from PostProcessor import TablesCreation, Epura
...
class SaprApp:
    ...
    def initialize_postprocess_section(self):
        ...
        build_diagrams_button = tk.Button(self.postprocess_frame, text="Построение эпюр", width=30, command=self.show_epur)
    ...
    def section_calc(self):
        ...
        if input_checker.input_is_correct(data):
            if self.x.get() == '' or self.bar_num.get() == '':
                ...
                u, n, sigma = main_processing.section_calc_for_window(self)
    ...
    def show_epur(self):
        ...
        data = self.user_input
        if input_checker.input_is_correct(data):
            x_coords, y_coords_nx, y_coords_sigma = main_processing.section_calc_for_nx_epur(data)
            epur.draw_epur_n_sigma(x_coords, y_coords_nx, "N")
            epur.draw_epur_n_sigma(x_coords, y_coords_sigma, "σ")
