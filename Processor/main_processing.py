import numpy as np
from PreProcessor import input_checker
...
def section_calc(root):
    root.refresh()
    data = root.user_input

    x = float(root.x.get())
    bar_num = int(root.bar_num.get())
    d0 = calculate_deltas(root)[bar_num - 1]
    dl = calculate_deltas(root)[bar_num]
